...
| 3 | **EC2 key pair** (SSH into the instances) | `aws ec2 create-key-pair --key-name kz-demo-app --query KeyMaterial --output text > kz-demo-app.pem && chmod 600 kz-demo-app.pem` |
...
First **create a new empty repository on GitHub** (e.g. `kz-demo-app`). Then copy this example out of
the course repo and wire it to yours:

```bash
git clone https://github.com/rbalman/devops-month.git
cp -R devops-month/examples/kz-demo-app kz-demo-app && cd kz-demo-app
...
```bash
git add -A && git commit -m "configure kz-demo-app" && git push -u origin main
```
